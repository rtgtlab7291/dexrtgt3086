...
Usage:
    cd robokit-internal
    uv run python examples/humanoid_retarget/verify_calibration.py
"""
...
# ---------------------------------------------------------------------------
# Calibration code path (from calibrate_config.py)
# ---------------------------------------------------------------------------
...
def calibration_apply_scale_and_offset(
    smplx_positions, smplx_quaternions, root_name, scale_table, link_mapping,
):
    """EXACT copy of calibrate_config.apply_scale_and_offset_np()."""
...
# ---------------------------------------------------------------------------
# Runtime code path (replicated from humanoid_retargeting_online.py)
# ---------------------------------------------------------------------------

def runtime_scale_and_offset(human_frame, config):
...
def test_math_match(config_path: str, height: float, assumption: float):
    ...
    # Calibration path (uses raw scale_table, no height adjustment)
    ...
    cal_result = calibration_apply_scale_and_offset(
        positions, quaternions, root_name, raw.get("scale_table", {}), cal_mapping,
    )

    # Runtime path (applies with_actual_height)
    rt_config = RetargetConfig.from_yaml(config_path)
    rt_config = rt_config.with_actual_height(height)
    rt_targets = runtime_scale_and_offset(frame, rt_config)
    ...
    for robot_link, human_joint, _, _, _, _ in cal_mapping:
        ...
        if cal_pos is not None and rt_data is not None:
            diff = np.linalg.norm(cal_pos - rt_data[0])
            max_diff = max(max_diff, diff)
    return max_diff


def test_height_invariance(config_path: str, heights: list):
    """Test that targets are height-invariant (after height adjustment,
    targets should be ~identical regardless of subject height).

    This is the key property: the robot's body doesn't change with human height.
    """
    import yaml
    with open(config_path) as f:
        raw = yaml.safe_load(f)
    ...
    for h in heights:
        height_ratio = h / assumption
        frame = make_frame_at_height(height_ratio)

        rt_config = RetargetConfig.from_yaml(config_path)
        rt_config = rt_config.with_actual_height(h)
        targets = runtime_scale_and_offset(frame, rt_config)
        ...
        if reference_targets is None:
            ...
            for hj, (ref_pos, _) in reference_targets.items():
                if hj in targets:
                    diff = np.linalg.norm(ref_pos - targets[hj][0])
                    max_diff = max(max_diff, diff)
...
def main():
    ...
    parser.add_argument("--config", type=str, default=str(SCRIPT_DIR / "ik_config/smplx_g1_calibrated_v2.yaml"))
    parser.add_argument("--original", type=str, default=str(SCRIPT_DIR / "ik_config/smplx_g1.yaml"))
    ...
    print(f"Original config:   {Path(args.original).name} (assumption={orig_assumption}m)")
    ...
    diff = test_math_match(args.config, cal_assumption, cal_assumption)
    status = "PASS" if diff < 1e-6 else "FAIL"
    print(f"  Calibrated @ h={cal_assumption}m: max diff = {diff:.8f}m  [{status}]")

    diff = test_math_match(args.original, orig_assumption, orig_assumption)
    status = "PASS" if diff < 1e-6 else "FAIL"
    ...
    cal_results = test_height_invariance(args.config, heights)
    ...
    print(f"  {'Height':>8s} {'Max diff from ref':>18s} {'Status':>8s}")
    orig_results = test_height_invariance(args.original, heights)
    ...
    for h in test_heights:
        height_ratio = h / cal_assumption
        frame = make_frame_at_height(height_ratio)
        rt_config = RetargetConfig.from_yaml(args.config)
        rt_config = rt_config.with_actual_height(h)
        targets = runtime_scale_and_offset(frame, rt_config)
